    ...
    <main className="relative min-h-screen overflow-x-hidden">
          ...
          <div className="grid md:grid-cols-3 gap-px bg-foreground/10">
            {plans.map((plan, idx) => (
              <ScrollReveal key={plan.name} direction="up" delay={idx * 100}>
                <div className={`relative p-8 lg:p-12 bg-background h-full ${plan.popular ? "md:-my-4 md:py-12 lg:py-16 border-2 border-foreground" : ""}`}>
                  {plan.popular && (
                  ...
                  <div className="mb-8">
                    ...
                    <h2 className="font-display text-3xl text-foreground mt-2">{plan.name}</h2>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-foreground/10">
                    <th className="text-left py-4 pr-8 font-normal text-muted-foreground w-1/2">Feature</th>
                ...
                <tbody>
                  {[
                    ...
                    <tr key={feature as string} className="border-b border-foreground/10 hover:bg-foreground/[0.02] transition-colors">
                      ...
                      {[starter, business, enterprise].map((val, i) => (
                        <td key={i} className="text-center py-4 px-4">
                          {typeof val === "boolean" ? (
                            val ? <Check className="w-4 h-4 mx-auto text-foreground" /> : <span className="text-foreground/20">—</span>
                          ) : (
        ...
        <div className="max-w-[1400px] mx-auto px-6 lg:px-12">
          <div className="grid lg:grid-cols-12 gap-16">
            <ScrollReveal direction="left" className="lg:col-span-4">
              <span className="font-mono text-xs tracking-widest text-muted-foreground uppercase block mb-6">FAQ</span>
            ...
            <div className="lg:col-span-8">
              {faqs.map((faq, i) => (
                <ScrollReveal key={i} direction="right" delay={i * 50}>
                  <div className="border-t border-foreground/10">
                    <button
                      onClick={() => setOpenFaq(openFaq === i ? null : i)}
                      className="w-full flex items-center justify-between py-6 text-left gap-8 group"
                    >
                      <span className="text-sm font-medium group-hover:text-muted-foreground transition-colors">{faq.q}</span>
                      <span className={`text-xl font-light shrink-0 transition-transform duration-300 ${openFaq === i ? "rotate-45" : ""}`}>+</span>
                    </button>
                    {openFaq === i && (
      ...
      <section className="py-32 border-t border-foreground/10 bg-foreground/[0.02]">
        <div className="max-w-[1400px] mx-auto px-6 lg:px-12 text-center">
          <ScrollReveal direction="up">
            <h2 className="font-display text-5xl lg:text-6xl tracking-tight mb-6">Not sure which plan?</h2>
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              ...
              <a href="/case-studies" className="inline-flex items-center justify-center gap-2 border border-foreground/20 px-8 h-14 text-base rounded-full hover:bg-foreground/5 transition-colors">
                See Client Results
              </a>
            </div>
          </ScrollReveal>
