...
const caseStudies = [
  {
    ...
    headline: "73% fewer support tickets across 50M customers",
    challenge:
    ...
    quote: "We went from drowning in tickets to having our team focus on genuinely complex cases. The multilingual sync alone saved us three full-time translators.",
  ...
  },
  {
    ...
    quote: "The 200K context window is what made this possible. We can inject our entire compliance rulebook into every single conversation. No other platform could do that.",
  ...
  },
  {
    ...
    quote: "Our customers in rural Thailand and Cambodia now get shipment updates in their own language before they even think to ask. That's a step change in service quality.",
    ...
    <main className="relative min-h-screen overflow-x-hidden">
      ...
      <section id="clients" className="py-32 lg:py-40">
        <div className="max-w-[1400px] mx-auto px-6 lg:px-12 space-y-32">
          {caseStudies.map((cs, idx) => (
            <ScrollReveal key={cs.id} direction="up" delay={idx * 60}>
              <article className="grid lg:grid-cols-12 gap-12">
                {/* Left: meta */}
                <div className="lg:col-span-4">
                  <div className="sticky top-32">
                    <span className="font-mono text-xs text-muted-foreground block mb-4">{cs.id}</span>
                    <h2 className="font-display text-4xl lg:text-5xl mb-3">{cs.client}</h2>
                    <p className="text-sm text-muted-foreground mb-2">{cs.country}</p>
                    <p className="text-sm text-muted-foreground mb-6">{cs.industry}</p>
                    <div className="flex flex-wrap gap-2 mb-8">
                      {cs.modules.map((m) => (
                        <span key={m} className="px-2 py-0.5 border border-foreground/20 text-xs font-mono">
                          Module {m}
                        </span>
                      ))}
                    </div>
                    <p className="text-xs font-mono text-muted-foreground leading-relaxed">{cs.tag}</p>
                  </div>
                </div>

                {/* Right: content */}
                <div className="lg:col-span-8">
                  <h3 className="font-display text-3xl lg:text-4xl mb-10 leading-tight">{cs.headline}</h3>

                  <div className="grid grid-cols-2 md:grid-cols-4 gap-px bg-foreground/10 mb-12">
                    {cs.results.map((r) => (
                      <div key={r.label} className="bg-background p-6 hover:bg-foreground/[0.02] transition-colors">
                  ...
                  <div className="space-y-8 mb-12">
                    <div>
                      <h4 className="text-xs font-mono text-muted-foreground uppercase tracking-widest mb-3">Challenge</h4>
                      <p className="text-muted-foreground leading-relaxed">{cs.challenge}</p>
                    </div>
                    <div>
                      <h4 className="text-xs font-mono text-muted-foreground uppercase tracking-widest mb-3">Solution</h4>
                  ...
                  <blockquote className="border-l-2 border-foreground pl-6">
                    ...
                    <cite className="text-sm text-muted-foreground not-italic">— {cs.quoteName}</cite>
                  </blockquote>
                </div>
              </article>
            </ScrollReveal>
      ...
      <section className="py-32 border-t border-foreground/10 bg-foreground/[0.02]">
        <div className="max-w-[1400px] mx-auto px-6 lg:px-12 text-center">
          <ScrollReveal direction="up">
            <h2 className="font-display text-5xl lg:text-6xl tracking-tight mb-6">Your results, next.</h2>
            <p className="text-muted-foreground mb-10 max-w-xl mx-auto">
              Most enterprise deployments go live within 6 weeks. Let's talk about your specific workflow.
            </p>
